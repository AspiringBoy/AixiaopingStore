...
import com.axp.axpseller.models.push.OtherLogin;
import com.axp.axpseller.models.push.SystemMsg;
import com.axp.axpseller.network.Push;
...
public class PushReceiver extends BroadcastReceiver {
    ...
    public void otherLogin(Context context, Push<OtherLogin> otherLoginPush) {
        if (otherLoginPush.getData().getUserId().equals(ContextParameter.getUserInfo().getAdminuserId())) {
    ...
    private void sendMessage(Context context, Push<Message> messagePush) {
        ...
        builder.setSmallIcon(isAboveLollipop ? R.drawable.icon_white : R.drawable.icon_app)
                ...
                .setTicker("爱小屏商家新消息哦~~")//提醒
                .setContentIntent(PendingIntent.getActivity(context, 0, intent, 0));
        Notification n = builder.getNotification();
        n.flags |= Notification.FLAG_AUTO_CANCEL;
        n.defaults = Notification.DEFAULT_SOUND;
//        n.sound = Uri.parse("android.resource://" + context.getPackageName() + "/" +R.raw.mm);
        nm.notify(0, n);
    }

    private void sendMessage(Push<SystemMsg> messagePush, Context context) {
        Log.d("雨落无痕丶", "sendMessage: uu");
        final NotificationManager nm = (NotificationManager) context.getSystemService(Context.NOTIFICATION_SERVICE);
        NotificationCompat.Builder builder = new NotificationCompat.Builder(context);
